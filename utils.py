...
def construct_datadicts(tokenizername, tokenizer, filepaths, testfile_names):
    ...
    if tokenizername.startswith('pdelobelle') or tokenizername.startswith("FacebookAI/roberta") or tokenizername.startswith('globertise'): #add spaces to each token when using robbert so that it marks subtokens during procesing
